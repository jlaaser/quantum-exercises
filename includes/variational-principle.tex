...
\begin{questions}
	\question Suppose we pick a trial state $\ket{\Psi_{trial}} = \sum_n c_n \ket{n}$.
		...
		\begin{parts}
			...
			\part Assuming that $E_0 < E_1 < E_2 < \dots$ (e.g. the energies increase as $n$ increases), what must be true about the relationship between $E_{trial}$ and $E_0$?  Fill in the correct inequality symbol (e.g $=$, $>$, $\geq$, $<$, $\leq$) to express your answer mathematically.
			
				\begin{equation*}
					E_{trial} \,\, \answerbox{30}\,\,  E_0
				\end{equation*}
				
				\vspace{0.1in}
			\part If we knew that $\ket{\Psi_{trial}}$ were \emph{orthogonal} to $\ket{0}$, what would have to be true about $c_0$?  What energy is the lower bound on $E_{trial}$ in this case?
	...
	\question With the relationships from the previous page in mind, we can revisit the helium atom.
		...
		\begin{parts}
			...
			(\emph{You should think about this question in purely chemical terms - no math or quantum needed here.  Hint: what phenomenon do we talk about when we describe e.g. why some protons have greater chemical shifts in NMR than others?})
