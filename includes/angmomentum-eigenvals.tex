\section*{Eigenvalues of the Angular Momentum Operators\sectionmark{Angular Momentum Eigenvalues}}
	...
	\begin{questions}
		...
		\question If we apply the raising operator to $\ket{Y_{c,b}}$, we increase the eigenvalue under $\hat M_z$ by $\hbar$, i.e. $\hat M_z \hat M_+ \ket{Y_{c,b}} = (b+\hbar) \hat M_+ \ket{Y_{c,b}} = (b+\hbar)\ket{Y_{c,b+1}}$
		...
		\question Our results from the previous page tell us that there is a maximum and minimum allowed value for $b$. Let's assume the maximum allowed value of $b$ is $b_{max}$, associated with state $\ket{Y_{c,b_{max}}}$, and the minimum allowed value is $b_{min}$, associated with state $\ket{Y_{c,b_{min}}}$.
